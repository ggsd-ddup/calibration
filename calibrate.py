...
result =[]
pos_beg = cps.HRIF_ReadActPos(0,0,result) #read current pose
print("Robot start at : "+"X =%f"+"Y =%f"+"Z =%f"+"RX =%f"+"RY =%f"+"RZ =%f",result[6],result[7],result[8],result[9],result[10],result[11])

workspace_limits = np.asarray([[-150, 150], [200, 500], [300, 400]]) # Cols: min max, Rows: x y z (define workspace limits in robot coordinates)
calib_grid_step = 50
...
PCS = [0,200,300,180,0,-90]
# Make robot gripper point upwards
cps.HRIF_WayPoint(0,0,1,PCS,Jonit_angle,TCP,UCS,acc,rad,0,0,0,0,0,0)
...
for calib_pt_idx in range(num_calib_grid_pts):
    ...
    cps.HRIF_WayPoint(0,0,1,tool_pose,Jonit_angle,TCP,UCS,vel,acc,rad,0,0,0,0,0,0)
    time.sleep(2)
    ...
    camera  = Camera()
    camera_color_img = camera.color_data.copy()
    camera_depth_img = camera.depth_data.copy()
